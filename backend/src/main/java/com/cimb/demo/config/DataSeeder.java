 ...
 *   customer  ->  user / Password1!   (role CUSTOMER)
...
public class DataSeeder implements CommandLineRunner {
    ...
    private static final String USER_TOTP = "JBSWY3DPEHPK3PXP";
    ...
    public void run(String... args) {
        if (users.count() == 0) {
            seedUser("user", "CIMB User", Role.CUSTOMER, "Password1!", USER_TOTP);
            seedUser("admin", "CIMB Admin", Role.ADMIN, "Admin123!", ADMIN_TOTP);
        }
        if (accounts.count() == 0) {
            accounts.save(new Account("acc-user-1", "user", "7048123945", "USER 1 ACCOUNT",
                    new BigDecimal("128450.75"), "010203-14-5678", "4111111111111111", false));
            accounts.save(new Account("acc-user-2", "user", "7098442156", "USER 2 ACCOUNT",
                    new BigDecimal("25489.20"), "010203-14-5678", "5500005555555559", false));
            accounts.save(new Account("acc-user-3", "user", "8012398765", "USER 3 ACCOUNT",
                    new BigDecimal("985420.10"), "920511-08-1234", "4000123412341234", false));
        }
